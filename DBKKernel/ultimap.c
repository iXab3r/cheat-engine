...
*/
{
	...
	LogInfo("ultimap(%I64x, %I64x, %d)", (UINT64)params->cr3, (UINT64)params->dbgctl_msr, params->DS_AREA_SIZE);
	DS_AREA[cpunr()]=NULL;
	...
	if (params->DS_AREA_SIZE)
	{
		...
		LogInfo("DS_AREA[%d]=%p", cpunr(), DS_AREA[cpunr()]);
...
{	
	...
	LogInfo("after KeInsertQueueApc");
