...
unsigned int vmx_redirect_interrupt1(VMXInterruptRedirectType redirecttype, unsigned int newintvector, unsigned int int1cs, UINT_PTR int1eip)
{
	...
	LogInfo("vmx_redirect_interrupt1: redirecttype=%d int1cs=%x int1eip=%llx sizeof(vmcallinfo)=%x", redirecttype, int1cs, int1eip, sizeof(vmcallinfo));
	vmcallinfo.structsize=sizeof(vmcallinfo);
	vmcallinfo.level2pass=vmx_password2;
...
unsigned int vmx_redirect_interrupt3(VMXInterruptRedirectType redirecttype, unsigned int newintvector, unsigned int int3cs, UINT_PTR int3eip)
{
	...
	LogInfo("vmx_redirect_interrupt3: int3cs=%x int3eip=%x sizeof(vmcallinfo)=%x", int3cs, int3eip, sizeof(vmcallinfo));
	vmcallinfo.structsize=sizeof(vmcallinfo);
	vmcallinfo.level2pass=vmx_password2;
...
unsigned int vmx_redirect_interrupt14(VMXInterruptRedirectType redirecttype, unsigned int newintvector, unsigned int int14cs, UINT_PTR int14eip)
{
	...
	LogInfo("vmx_redirect_interrupt14: int14cs=%x int14eip=%x sizeof(vmcallinfo)=%x", int14cs, int14eip, sizeof(vmcallinfo));
	vmcallinfo.structsize=sizeof(vmcallinfo);
	vmcallinfo.level2pass=vmx_password2;
...
unsigned int vmx_exit_cr3_callback(unsigned int newcr3)
{
	...
	//LogInfo("vmx_exit_cr3_callback(%x)",newcr3);
...
unsigned int vmx_ultimap(UINT_PTR cr3towatch, UINT64 debugctl_value, void *storeaddress)
{
	...
	LogInfo("vmx_ultimap(%I64x, %I64x, %I64x)", (UINT64)vmcallinfo.cr3, (UINT64)vmcallinfo.debugctl, vmcallinfo.storeaddress);
...
#pragma pack()
	...
	LogInfo("vmx_add_memory(%p,%d)", list, count);
	LogInfo("vmx_add_memory(vmx_password1=%x,vmx_password2=%x)", vmx_password1, vmx_password2);

	LogInfo("structsize at offset %d", (UINT64)(&vmcallinfo->structsize) - (UINT64)vmcallinfo);
	LogInfo("level2pass at offset %d", (UINT64)(&vmcallinfo->level2pass) - (UINT64)vmcallinfo);
	LogInfo("command at offset %d", (UINT64)(&vmcallinfo->command) - (UINT64)vmcallinfo);
	LogInfo("PhysicalPages[0] at offset %d", (UINT64)(&vmcallinfo->PhysicalPages[0]) - (UINT64)vmcallinfo);
	LogInfo("PhysicalPages[1] at offset %d", (UINT64)(&vmcallinfo->PhysicalPages[1]) - (UINT64)vmcallinfo);
	...
	__try
	{
		...
		LogInfo("vmcallinfo->structsize=%d", vmcallinfo->structsize);
		vmcallinfo->level2pass = vmx_password2;
	...
	__except (1)
	{
		LogInfo("vmx_add_memory(%p,%d) gave an exception at part %d with exception code %x", list, count, j, GetExceptionCode());		
