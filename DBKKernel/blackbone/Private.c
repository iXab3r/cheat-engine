...
{
	LIST_ENTRY					Node;
	CHAR						SyscallName[300];
	ULONG						SyscallIndex;
} SYSCALL_INFO, *PSYSCALL_INFO;


static LIST_ENTRY g_SyscallInfoListHead = { 0 };
...
static
NTSTATUS
InitializeNtSyscallIndexesInternal(
    ...
    _Out_ PLIST_ENTRY SyscallList)
{
    ...
    // Accept PE32 or PE32+ (some toolchains used a single macro in the C++ you pasted)
    ...
    if (ExportDataDirectory->VirtualAddress == 0 || ExportDataDirectory->Size == 0)
    {
    	LogInfo("BlackBone: %s: Base address %p does not seem to be a valid PE header or export directory is malformed, VA %p, Size %d",
				 ...
				 ExportDataDirectory->Size);
	    return STATUS_UNSUCCESSFUL;
    }

    PIMAGE_EXPORT_DIRECTORY ExportDirectory = (PIMAGE_EXPORT_DIRECTORY)(NtdllBaseAddress + ExportDataDirectory-> VirtualAddress);
    PULONG ExportName = (PULONG)(NtdllBaseAddress + ExportDirectory->AddressOfNames);
    ...
    for (ULONG Count = 0; Count < ExportDirectory->NumberOfNames; Count++) {
	    PCHAR CurrentExportName = (PCHAR)(NtdllBaseAddress + ExportName[Count]);
        PUCHAR CurrentExportAddress = (PUCHAR)(NtdllBaseAddress + ExportAddress[ExportOrdinal[Count]]);

        // Only exports starting with "Nt"/"Zw"
        if (!((CurrentExportName[0] == 'N' && CurrentExportName[1] == 't') ||
              (CurrentExportName[0] == 'Z' && CurrentExportName[1] == 'w'))) {
            continue;
        }

        // Walk the first few bytes, look for: mov eax, imm32 (B8 xx xx xx xx)
    	auto syscallInfoSize = sizeof(SYSCALL_INFO);
        for (ULONG ByteCount = 0;
             ByteCount < 32 &&
             (CurrentExportAddress + ByteCount) < (NtdllBaseAddress + NtHeaders->OptionalHeader.SizeOfImage);
             ByteCount++)
        {
            UCHAR op = CurrentExportAddress[ByteCount];

            // probable end of tiny stub
            if (op == 0xC3 || op == 0xC2)
            {
	            break;
            }
                  ...
                  *(PULONG)(CurrentExportAddress + ByteCount + 1));
            {
                ...
                RtlZeroMemory(NewSyscallInfo, sizeof(SYSCALL_INFO));

                // Extract index from imm32 after B8
                NewSyscallInfo->SyscallIndex = *(PULONG)(CurrentExportAddress + ByteCount + 1);

                // Copy name safely (truncate + NUL)
                // Option A: CRT-like
                // _snprintf(NewSyscallInfo->SyscallName, ARRAYSIZE(NewSyscallInfo->SyscallName), "%s", CurrentExportName);
                // Option B: NT-safe copy
                RtlStringCbCopyNA(NewSyscallInfo->SyscallName,
                                  sizeof(NewSyscallInfo->SyscallName),
                                  CurrentExportName,
                                  strlen(CurrentExportName));

                InsertTailList(SyscallList, &NewSyscallInfo->Node);
            }

            // You can break here if you assume the first B8 is “the one”
            break;
        }
    }

    return IsListEmpty(SyscallList) ? STATUS_NOT_FOUND : STATUS_SUCCESS;
}

// ---------------- top-level init (C version) ----------------

NTSTATUS
InitializeNtSyscallIndexes(
    _Out_ PLIST_ENTRY SyscallList
)
{
    ...
    HANDLE            NtdllSectionHandle = NULL;
    PVOID             NtdllBaseAddress   = NULL;
    LARGE_INTEGER     SectionOffset;
    SIZE_T            ViewSize = 0;

    InitializeListHead(SyscallList);

    InitializeObjectAttributes(&ObjAttr, &KnownNtdllUS, OBJ_CASE_INSENSITIVE, NULL, NULL);
    RtlZeroMemory(&SectionOffset, sizeof(SectionOffset));

    // Open KnownDlls\ntdll.dll section (read-only)
    ...
    Status = ZwMapViewOfSection(
        NtdllSectionHandle,
        ZwCurrentProcess(),          // pseudo-handle is fine for mapping the "current" process
        &NtdllBaseAddress,
        0,
        0,
        &SectionOffset,
    ...
    Status = InitializeNtSyscallIndexesInternal((PUCHAR)NtdllBaseAddress, SyscallList);

Cleanup:
    // Unmap: **ZwUnmapViewOfSection takes the PROCESS handle and BaseAddress**
    if (NtdllBaseAddress != NULL) {
        ZwUnmapViewOfSection(ZwCurrentProcess(), NtdllBaseAddress);
        NtdllBaseAddress = NULL;
    }

    if (NtdllSectionHandle) {
        ZwClose(NtdllSectionHandle);
        NtdllSectionHandle = NULL;
    }

    return Status;
}

NTSTATUS InitializeNtSyscalls(void)
{
	return InitializeNtSyscallIndexes(&g_SyscallInfoListHead ); 
}

// ---------------- lookup helper (C version) ----------------

ULONG
GetNtSyscallIndex(
    _In_ PCSTR SyscallName
)
{
    PLIST_ENTRY   Entry;
    ANSI_STRING   Target;
    if (SyscallName == NULL || IsListEmpty(&g_SyscallInfoListHead))
    {
	    return MAXULONG;
    }

    RtlInitAnsiString(&Target, SyscallName);

    for (Entry = g_SyscallInfoListHead.Flink;
         Entry != &g_SyscallInfoListHead;
         Entry = Entry->Flink)
    {
        PSYSCALL_INFO Info = CONTAINING_RECORD(Entry, SYSCALL_INFO, Node);
        ANSI_STRING    Cur;
        RtlInitAnsiString(&Cur, Info->SyscallName);

        if (RtlEqualString(&Target, &Cur, TRUE)) {
            return Info->SyscallIndex;
        }
    }
    return MAXULONG;
}
