    ...
    case IRP_MJ_DEVICE_CONTROL:
        {
            ...
            case IOCTL_BLACKBONE_MAP_DRIVER:
                {
                    if (inputBufferLength >= sizeof(MMAP_DRIVER) && ioBuffer)
                    {
                        ...
                        RtlCopyMemory(buf, ((PMMAP_DRIVER)ioBuffer)->FullPath,
                                      sizeof(((PMMAP_DRIVER)ioBuffer)->FullPath));
            ...
            case IOCTL_BLACKBONE_ENUM_REGIONS:
                {
                    if (inputBufferLength >= sizeof(ENUM_REGIONS) && outputBufferLength >= sizeof(ENUM_REGIONS_RESULT)
                        && ioBuffer)
                    {
                        ULONG count = (outputBufferLength - sizeof(ULONGLONG)) / sizeof(MEMORY_BASIC_INFORMATION);
                        PENUM_REGIONS_RESULT pResult =
                            ExAllocatePoolWithTag(PagedPool, outputBufferLength, BB_POOL_TAG);
                        pResult->count = count;
                        ...
                        if (NT_SUCCESS(Irp->IoStatus.Status))
                        {
                            Irp->IoStatus.Information = sizeof(pResult->count) + pResult->count * sizeof(
