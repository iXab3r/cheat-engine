...
UINT64 PhysicalMemoryRanges = 0;
//initialized once, and used thereafter. If the user adds/removes ram at runtime, screw him and make him the reload the driver
...
void mykapc2(PKAPC Apc, PKNORMAL_ROUTINE NormalRoutine, PVOID NormalContext, PVOID SystemArgument1,
             PVOID SystemArgument2)
{
    ...
    if (ZwQueryInformationProcess(ZwCurrentProcess(), ProcessWow64Information, &iswow64, sizeof(iswow64), NULL) ==
        STATUS_SUCCESS)
    {
#if (NTDDI_VERSION >= NTDDI_VISTA)
        if (iswow64)
...
void mykapc(PKAPC Apc, PKNORMAL_ROUTINE NormalRoutine, PVOID NormalContext, PVOID SystemArgument1,
            PVOID SystemArgument2)
{
...
#if (NTDDI_VERSION >= NTDDI_VISTA)
OB_PREOP_CALLBACK_STATUS ThreadPreCallback(PVOID RegistrationContext,
                                           POB_PRE_OPERATION_INFORMATION OperationInformation)
{
    ...
    {
        if ((PsGetProcessId(DRMProcess) == PsGetThreadProcessId(OperationInformation->Object)) || ((DRMProcess2) && (
            PsGetProcessId(DRMProcess2) == PsGetThreadProcessId(OperationInformation->Object))))
        {
            //probably block it
            ...
            if (OperationInformation->Operation == OB_OPERATION_HANDLE_CREATE)
            {
                ...
                LogInfo("PID %d opened a handle to the a CE thread with access mask %x", PsGetCurrentProcessId(), da);
            ...
            else if (OperationInformation->Operation == OB_OPERATION_HANDLE_DUPLICATE)
            {
                ...
                LogInfo("PID %d duplicated a handle to a CE thread with access mask %x", PsGetCurrentProcessId(), da);
...
{
    //LogInfo("ProcessPostCallback");
}


OB_PREOP_CALLBACK_STATUS ProcessPreCallback(PVOID RegistrationContext,
                                            POB_PRE_OPERATION_INFORMATION OperationInformation)
{
    if (DRMProcess == NULL)
        return OB_PREOP_SUCCESS;
    ...
    if (OperationInformation->ObjectType == *PsProcessType)
    {
        if ((OperationInformation->Object == DRMProcess) || (OperationInformation->Object == DRMProcess2))
        {
            ...
            if (OperationInformation->Operation == OB_OPERATION_HANDLE_CREATE)
            {
                ...
                LogInfo("PID %d(%p) opened a handle to the CE process(%p) with access mask %x", PsGetCurrentProcessId(),
                        PsGetCurrentProcess(), DRMProcess, da);
...
BOOL DispatchIoctlDBVM(IN PDEVICE_OBJECT DeviceObject, ULONG IoControlCode, PVOID lpInBuffer, DWORD nInBufferSize,
                       PVOID lpOutBuffer, DWORD nOutBufferSize, PDWORD lpBytesReturned)
/*
...
NTSTATUS DispatchIoctl(IN PDEVICE_OBJECT DeviceObject, IN PIRP Irp)
{
    ...
    {
        LogInfo("DispatchIoctl called by a process without SeDebugPrivilege");
    ...
    case IOCTL_CE_READMEMORY:
        __try
        {
            ...
            ntStatus = ReadProcessMemory((DWORD)pinp->processid,NULL, (PVOID)(UINT_PTR)pinp->startaddress,
                                         pinp->bytestoread, pinp)
                           ? STATUS_SUCCESS
                           : STATUS_UNSUCCESSFUL;
        }
        __except (1)
    ...
    case IOCTL_CE_WRITEMEMORY:
        {
            __try
            {
                ...
                pinp = Irp->AssociatedIrp.SystemBuffer;
                ntStatus = WriteProcessMemory((DWORD)pinp->processid,NULL, (PVOID)(UINT_PTR)pinp->startaddress,
                                              pinp->bytestowrite, (PVOID)((UINT_PTR)pinp + sizeof(inp)))
                               ? STATUS_SUCCESS
                               : STATUS_UNSUCCESSFUL;
            }
            __except (1)
    ...
    case IOCTL_CE_OPENPROCESS:
        {
            ...
            {
                //LogInfo("ProcessHandle=%x", (int)ProcessHandle);
                POutput->Special = 1;
            }
    ...
    case IOCTL_CE_QUERY_VIRTUAL_MEMORY:
        {
            ...
            __try
            {
                ntStatus = GetMemoryRegionData((DWORD)PInputBuf->ProcessID, NULL,
                                               (PVOID)(UINT_PTR)(PInputBuf->StartAddress), &(POutputBuf->protection),
                                               &length, &BaseAddress);
            }
            __except (1)
            {
                LogInfo("GetMemoryRegionData error");
                ntStatus = STATUS_UNSUCCESSFUL;
                break;
            }
    ...
    case IOCTL_CE_GETPETHREAD:
        {
            *(PUINT64)Irp->AssociatedIrp.SystemBuffer = (UINT64)getPEThread(
                (UINT_PTR)*(PULONG)Irp->AssociatedIrp.SystemBuffer);
            ntStatus = STATUS_SUCCESS;
            break;
        }
    ...
    case IOCTL_CE_READPHYSICALMEMORY:
        {
            ...
            LogInfo("IOCTL_CE_READPHYSICALMEMORY:pinp->startaddress=%x, pinp->bytestoread=%d", pinp->startaddress,
                    pinp->bytestoread);
    ...
    case IOCTL_CE_GETPHYSICALADDRESS:
        {
            ...
            if (ntStatus == STATUS_SUCCESS)
            {
                //LogInfo("physical.LowPart=%x",physical.LowPart);
    ...
    case IOCTL_CE_GETCR3:
        {
            ...
            if (PsLookupProcessByProcessId((PVOID)(UINT_PTR)(*(ULONG*)Irp->AssociatedIrp.SystemBuffer),
                                           &selectedprocess) == STATUS_SUCCESS)
            {
                __try
                {
                    KAPC_STATE apc_state;
    ...
    case IOCTL_CE_GETIDT:
        {
            ...
            //so that the 32-bit version doesn't have to deal with garbage at the end
    ...
    case IOCTL_CE_USERDEFINEDINTERRUPTHOOK:
        {
            ...
            inthook_HookInterrupt((unsigned char)(pinp->interruptnumber), (int)pinp->newCS, (ULONG_PTR)pinp->newRIP,
                                  (PJUMPBACK)(UINT_PTR)(pinp->addressofjumpback));
...
#else
                CreateProcessNotifyRoutineEnabled = (PsSetCreateProcessNotifyRoutine(CreateProcessNotifyRoutine, FALSE)
                    == STATUS_SUCCESS);
#endif
                if (CreateProcessNotifyRoutineEnabled)
                    CreateThreadNotifyRoutineEnabled = (PsSetCreateThreadNotifyRoutine(CreateThreadNotifyRoutine) ==
                        STATUS_SUCCESS);
            }
    ...
    case IOCTL_CE_GETPROCESSEVENTS:
        {
            ...
            *(PUCHAR)Irp->AssociatedIrp.SystemBuffer = ProcessEventCount;
            RtlCopyMemory((PVOID)((UINT_PTR)Irp->AssociatedIrp.SystemBuffer+1), &ProcessEventdata[0],
                          ProcessEventCount*sizeof(ProcessEventdta));
    ...
    case IOCTL_CE_GETTHREADEVENTS:
        {
            ...
            *(PUCHAR)Irp->AssociatedIrp.SystemBuffer = ThreadEventCount;
            RtlCopyMemory((PVOID)((UINT_PTR)Irp->AssociatedIrp.SystemBuffer+1), &ThreadEventData[0],
                          ThreadEventCount*sizeof(ThreadEventDta));
            ...
            if (PsLookupProcessByProcessId((PVOID)(UINT_PTR)(inp->ProcessID), &selectedprocess) == STATUS_SUCCESS)
            {
                __try
                {
                    ...
                    __try
                    {
                        ...
                        ntStatus = ZwAllocateVirtualMemory((HANDLE)-1, &BaseAddress, 0, &RegionSize,
                                                           (ULONG)inp->AllocationType, (ULONG)inp->Protect);
                ...
                if (inp->ToPID)
                {
                    ...
                    if (PsLookupProcessByProcessId((PVOID)(UINT_PTR)(inp->ToPID), &selectedprocess) == STATUS_SUCCESS)
                    {
                        __try
                        {
                            ...
                            __try
                            {
                                outp->Address = (UINT64)MmMapLockedPagesSpecifyCache(
                                    FromMDL, UserMode, MmWriteCombined, NULL, FALSE, NormalPagePriority);
                ...
                __try
                {
                    ...
                    if (mdl)
                    {
                        __try
                        {
                            ...
                            LogInfo("MmProbeAndLockPages succeeded");
                        }
                        __except (1)
                        {
                            LogInfo("MmProbeAndLockPages failed");
                            IoFreeMdl(mdl);
                            ntStatus = STATUS_UNSUCCESSFUL;
                            break;
                        }
    ...
    case IOCTL_CE_UNLOCK_MEMORY:
        {
            ...
            LogInfo("IOCTL_CE_UNLOCK_MEMORY");
            inp = Irp->AssociatedIrp.SystemBuffer;
    ...
    case IOCTL_CE_GETPROCESSNAMEADDRESS:
        {
            ...
            {
                LogInfo("PsGetProcessImageFileName==NULL");
                ntStatus = STATUS_UNSUCCESSFUL;
            }
    ...
    case IOCTL_CE_WRITESIGNOREWP:
        {
            ...
            LogInfo("[IOCTL_CE_WRITESIGNOREWP] Updating KernelWritesIgnoreWP: %d => %d", KernelWritesIgnoreWP,
                    newValue);
    ...
    case IOCTL_CE_GD_SETBREAKPOINT:
        {
            ...
            if (inp->active)
            {
                ...
                ntStatus = debugger_setGDBreakpoint(inp->debugregspot, (UINT_PTR)inp->address,
                                                    (BreakType)inp->breakType, (BreakLength)inp->breakLength);
            }
            else
            {
    ...
    case IOCTL_CE_ULTIMAP2:
        {
            ...
            LogInfo("IOCTL_CE_ULTIMAP2");
            for (i = 0; i < (int)(inp->RangeCount); i++)
                LogInfo("%d=%p -> %p", i, (PVOID)(UINT_PTR)inp->Ranges[i].StartAddress,
                    (PVOID)(UINT_PTR)inp->Ranges[i].EndAddress);

            SetupUltimap2(inp->PID, inp->Size, inp->OutputPath, inp->RangeCount, inp->Ranges, inp->NoPMI, inp->UserMode,
                          inp->KernelMode);
...
#pragma pack()
            ...
            ntStatus = ultimap(inp->targetCR3, inp->dbgctl, (int)inp->dsareasize, inp->savetofile, &inp->filename[0],
                               inp->HandlerCount);
    ...
    case IOCTL_CE_INITIALIZE:
        {
            ...
            ObReferenceObjectByHandle((HANDLE)(UINT_PTR)pinp->ProcessEvent, EVENT_ALL_ACCESS, NULL, KernelMode,
                                      &ProcessEvent, NULL);
            ObReferenceObjectByHandle((HANDLE)(UINT_PTR)pinp->ThreadEvent, EVENT_ALL_ACCESS, NULL, KernelMode,
                                      &ThreadEvent, NULL);
...
#pragma pack()
    ...
    case IOCTL_CE_ENABLE_DRM:
        {
            ...
            LogInfo("inp->PreferedAltitude=%p", inp->PreferedAltitude);
            LogInfo("inp->PreferedAltitude=%p", inp->ProtectedProcess);
            ...
            LogInfo("DRMProcess=%p", DRMProcess);
            LogInfo("DRMProcess2=%p", DRMProcess2);
            ...
            if (DRMHandle == NULL)
            {
                ...
                LogInfo("Activating CE's super advanced DRM"); //yeah right....

                LogInfo("RandomVal=%d", RandomVal);
                RtlStringCbPrintfW(wcAltitude, sizeof(wcAltitude) - 2, L"%d", RandomVal);

                LogInfo("wcAltitude=%S", wcAltitude);
    ...
    case IOCTL_CE_GET_PEB:
        {
            ...
            //LogInfo("IOCTL_CE_GET_PEB");
            KeStackAttachProcess((PKPROCESS)ep, &oldstate);
            __try
            {
                ULONG r;
                PROCESS_BASIC_INFORMATION pbi;
                //LogInfo("Calling ZwQueryInformationProcess");
                ntStatus = ZwQueryInformationProcess(ZwCurrentProcess(), ProcessBasicInformation, &pbi, sizeof(pbi),
                                                     &r);
                if (ntStatus == STATUS_SUCCESS)
                {
                    ...
                    *(QWORD*)Irp->AssociatedIrp.SystemBuffer = (QWORD)(pbi.PebBaseAddress);
                    ...
                    LogInfo("ZwQueryInformationProcess failed");
            }
            __finally
            {
    ...
    case IOCTL_CE_QUERYINFORMATIONPROCESS:
        {
            ...
            PEPROCESS selectedprocess;
            LogInfo("IOCTL_CE_QUERYINFORMATIONPROCESS");
            ...
            __try
            {
                if (PsLookupProcessByProcessId((HANDLE)(UINT_PTR)inp->processid, &selectedprocess) == STATUS_SUCCESS)
                {
                    ...
                    __try
                    {
                        ...
                        {
                            LogInfo("NULL ProcessInformationAddress");
                            outp->result = ZwQueryInformationProcess(
                                NtCurrentProcess(), inp->ProcessInformationClass, NULL,
                                (ULONG)inp->ProcessInformationLength, &returnLength);
                        }
                        else
                            outp->result = ZwQueryInformationProcess(
                                NtCurrentProcess(), inp->ProcessInformationClass, &(outp->data),
                                (ULONG)inp->ProcessInformationLength, &returnLength);

                        LogInfo("outp->result=%x", outp->result);

                        outp->returnLength = returnLength;
                        LogInfo("outp->returnLength=%x", outp->returnLength);
                ...
                {
                    LogInfo("Failed to find pid %x", inp->processid);
    ...
    case IOCTL_CE_ALLOCATE_MEMORY_FOR_DBVM:
        {
            ...
            mdl = MmAllocatePagesForMdlEx(LowAddress, HighAddress, SkipBytes, (SIZE_T)pagecount * 4096, MmCached,
                                          MM_ALLOCATE_REQUIRE_CONTIGUOUS_CHUNKS | MM_ALLOCATE_FULLY_REQUIRED);
            //do not free this, EVER
            if (mdl)
            {
                int i;
                ...
                if (pfnlist)
                {
                    ...
                    if (mi)
                    {
                        ...
                        if (mi->List)
                        {
                            ...
                            LogInfo("Failure allocating mi->List");
                        ExFreePool(mi);
                    }
                    else
                        LogInfo("Failure allocting mi");
                }
                else
                    LogInfo("Failure getting pfn list");
            ...
            {
                LogInfo("Failure allocating MDL");
    ...
    case IOCTL_CE_CREATEREMOTETHREAD:
        {
            // Input contract (METHOD_BUFFERED):
            // struct { UINT64 processid; PVOID startaddress; PVOID parameter; };
            // Output contract: struct { HANDLE ThreadProcessId; HANDLE ThreadId; };

            typedef struct _CRT_INPUT
            {
                UINT64 processid;
                PVOID startaddress;
                PVOID parameter;
            } CRT_INPUT, *PCRT_INPUT;

            typedef struct _CRT_OUTPUT
            {
                HANDLE ThreadProcessId;
                HANDLE ThreadId;
            } CRT_OUTPUT, *PCRT_OUTPUT;
            ...
            if (inLen < sizeof(CRT_INPUT) || outLen < sizeof(CRT_OUTPUT))
            {
                LogWarn("[IOCTL_CE_CREATEREMOTETHREAD] Buffer too small: in=%lu(out of %zu) out=%lu(out of %zu)",
                        inLen, sizeof(CRT_INPUT), outLen, sizeof(CRT_OUTPUT));
            ...
            {
                LogWarn("[IOCTL_CE_CREATEREMOTETHREAD] Inject_CreateRemoteThread failed: 0x%08X", ntStatus);
    ...
    case IOCTL_BLACKBONE_INJECT_DLL:
        {
            LogInfo("[IOCTL_BLACKBONE_INJECT_DLL] Rerouting to BB");
            return BBDispatch(DeviceObject, Irp);
        }
